...
namespace AzureTableDataStore
{
    ...
    public class TableDataStore<TData> : ITableDataStore<TData> where TData : new()
    {
        ...
        public string BuildBlobPath(ReflectionUtils.PropertyRef<LargeBlob> blobPropRef, string partitionKey,
            string rowKey)
        {
        ...
        public string BuildBlobPath(string storageTableName, string partitionKey, string rowKey,
            ...
            string etag)
        {
            try
            {
                ...
                try
                {
                    converted = EntityPropertyConverter.ConvertBack<TData>(properties, EntityPropertyConverterOptions, null);

                }
                catch (NullReferenceException)
                {
                    //the issue with old version not yet updated such as mobile, or web

                    var instance = Activator.CreateInstance<TData>();
                    var flatteredAsPerThisVersion = ExtractEntityProperties(instance);

                    var propsToRemove = new Dictionary<string, EntityProperty>();

                    foreach (var property in properties)
                    {
                        if (!flatteredAsPerThisVersion.PropertyDictionary.Any(x => x.Key == property.Key))
                        {
                            if (property.Key == _configuration.RowKeyProperty || property.Key == _configuration.PartitionKeyProperty)
                            {

                            }
                            else
                            {
                                propsToRemove.Add(property.Key, property.Value);
                    ...
                    foreach (var property in properties)
                    {
                        PropertyInfo propertyInfo = typeof(TData).GetProperty(property.Key);
                        if (propertyInfo != null)
                        {
                            //var value = Convert.ChangeType(property.Value.PropertyAsObject, propertyInfo.PropertyType);
                            ////propertyInfo.SetValue(seed, value, null);

                            //if (!propsToRemove.Any(x => x.Key == property.Key))
                            //{
                        ...
                        else
                        {
                            ...
                            // Handle properties that don't exist in the older version
                            // This could involve ignoring them or mapping them to alternative properties
                        }

                    }


                    foreach (var propertyToRemove in propsToRemove)
                    {
                ...
                //var converted = ConvertBackCustom2<TData>(properties, EntityPropertyConverterOptions, null);
        ...
        public static TData ConvertBackCustom<TData>(IDictionary<string, EntityProperty> flattenedEntityProperties, EntityPropertyConverterOptions entityPropertyConverterOptions, OperationContext operationContext)
        {
            ...
            foreach (var property in flattenedEntityProperties)
            {
                PropertyInfo propertyInfo = typeof(TData).GetProperty(property.Key);
                if (propertyInfo != null)
                {
            ...
            return EntityPropertyConverter.ConvertBack<TData>(newProps, entityPropertyConverterOptions, null);
            //T seed = (T)Activator.CreateInstance(typeof(T));
            //return newProps.Aggregate(seed, (T current, KeyValuePair<string, EntityProperty> kvp) => (T)SetProperty(current, kvp.Key, kvp.Value.PropertyAsObject, entityPropertyConverterOptions, operationContext));
        }


        public static TData ConvertBackCustom2<TData>(IDictionary<string, EntityProperty> flattenedEntityProperties, EntityPropertyConverterOptions entityPropertyConverterOptions, OperationContext operationContext)
        {
            if (flattenedEntityProperties == null)
            {
                return default(TData);
            ...
            var instance = Activator.CreateInstance<TData>();
            var flatteredAsPerThisVersion = EntityPropertyConverter.Flatten(instance, entityPropertyConverterOptions, null);

            var newProps = new Dictionary<string, EntityProperty>();

            foreach (var property in flattenedEntityProperties)
            {
                if (flatteredAsPerThisVersion.Any(x => x.Key == property.Key))
                {
                    newProps.Add(property.Key, property.Value);
                }
            }
