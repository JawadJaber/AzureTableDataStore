...
namespace AzureTableDataStore.Tests.IntegrationTests
{
    ...
    public class SingleOperationsTestSuite : IClassFixture<StorageContextFixture>
    {
        ...
        [Fact(/*Skip = "reason"*/)]
        public async Task T07_IgnoreProperty_InsertOrReplace_One()
        {
            var store = _fixture.GetNewTableDataStore<TelescopePackageProduct_WithIgnoreProperty>("base");

            // Same row and partition key as the previously created instance.
            var item = MockData.TelescopeMockDataGenerator.CreateDataSet(1).Select(x=> TelescopePackageProduct_WithIgnoreProperty.Create(x)).First();
